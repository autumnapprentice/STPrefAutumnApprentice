    ...
    def run(self, view, errors, highlights):
        ...
        stylesheet = '''
            <style>
                ...
                #inline-error {
                    background-color: color(var(--foreground) blend(grey 80%));
                    border-radius: 5px;
                }
                ...
                div.error {
                    background-color: color(var(--background) blend(red 75%));
                }

                div.warning {
                    background-color: color(var(--background) blend(orange 70%));
                }
                ...
                div.error a {
                    color: color(var(--foreground) blend(red 75%));
                }

                div.warning a {
                    color: color(var(--foreground) blend(orange 70%));
                }
            </style>
        '''
        ...
        for pt, msg in options:
            divclass = "warning" if highlights.line_type(view.rowcol(pt)[0]) == "warning" else "error"

            phantom_content = """
               {stylesheet}
               <body id="inline-error">
                  <div class="{divclass}">
                      <a href="hide"><span class="message">{msg}</span></a>
                  </div>
              </body>
            """.format(**locals())
